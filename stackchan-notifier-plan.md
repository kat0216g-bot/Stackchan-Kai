...
## 長文の吹き出し見切れ対策（2026-07-20）

株取引プロジェクトからのスクリーニング結果通知で「長い文章だと見切れる」と判明。
`m5stack-avatar`ライブラリの吹き出し(`Balloon`)描画が**1行決め打ち**だったことが原因。

### 原因
`C:\Users\kat02\OneDrive\ドキュメント\Arduino\libraries\M5Stack_Avatar\src\Balloon.h`の
`Balloon::draw()`は、テキスト全体を`drawString()`で1行として描画し、吹き出しの楕円は
テキスト幅に応じて横に伸びるだけだった。画面幅を超えると単純に見切れる。
**このライブラリはリポジトリ外（PC共有のArduinoライブラリ）**なので、修正内容を
`firmware/library_patches/Balloon.h`にバックアップ保存した（ライブラリ再インストール時の
消失対策）。実際に使うには引き続き上記インストール先パスに配置する必要がある。

### 実装内容
1. **`stackchan_notify.py`**: `wrap_speech()`を追加。半角=1単位・全角=2単位の概算で、
   `WRAP_MAX_UNITS_PER_LINE=24`（全角约12文字相当）×`WRAP_MAX_LINES=3`行を超える分は
   末尾を「…」で省略し、`\n`区切りの複数行テキストに変換。`build_payload()`内で自動適用
   （`--event`・`speak()`・CLI汎用モードすべてに共通で効く）。
2. **`Balloon.h`（ライブラリパッチ）**: `\n`で分割して複数行描画できるよう改修。
   - 行間は固定値ではなく`M5.Lcd.fontHeight()`で実フォント高さを取得して計算
     （固定値だと実フォントより小さく、複数行が重なって読めなくなる不具合があった）。
   - 行が増えた分は**下ではなく上に**広げる（吹き出しの基準位置`cy`が画面下寄りのため、
     下に伸ばすと3行目以降が画面外にはみ出る不具合があった）。最終行の位置を1行時と
     同じ`cy`に固定し、それ以前の行を上へ積み上げる形にした。

### 実機確認結果
3行の長文（本日の株式スクリーニング結果...）を送信し、見切れず・重ならず・画面内に
収まって表示されることを確認。ただし**吹き出しが顔のかなりの部分を覆う**ため、
実用上は設定した上限（全角約36文字程度）を超える長文はなるべく避けるのが望ましい、
とユーザーからのフィードバックあり。

### 申し送り
- `Balloon.h`の修正はPC上のライブラリファイルへの直接編集であり、**Gitでは追跡されない**。
  別PCや再セットアップ時は`firmware/library_patches/Balloon.h`の内容を
  `M5Stack_Avatar`ライブラリの同名ファイルに上書き配置する必要がある。
- 行数・1行文字数の上限は実機で調整した値（フォント: `lgfxJapanGothic_12`, `TEXT_SIZE=2`固定）。
  フォントを変える場合は再調整が必要。

---

## AquesTalk-ESP32 音声合成の検討（調査のみ・未実装）

「スタックチャンに実際に声を出させる」ことができるか、ユーザーからの相談で調査した
（実装はせず検討のみ）。

### 概要
- 提供元: AQUEST社。漢字かな混じりテキストからESP32単体でリアルタイム音声合成できる
  組み込み向けライブラリ（クラウド・Wi-Fi音声合成サービス不要）。
- 対応: ESP32-DevKitC, M5Stack Core, M5Stack CoreS3, M5StampS4 で動作実績あり。
  2024年10月版でESP32-S3にも正式対応。Arduino core for ESP32 / ESP-IDF で利用可能。
- 必要リソース: ROM 200KB・RAM 21KB（辞書データ除く）。音声記号列のみならROM 28KB・RAM 500Bまで削減可。
- 辞書データ: 通常版7MB(約38万語) / Small版2MB(約10万語)。SPIフラッシュやSDカード等の
  外部メモリに配置可能（本体RAMを圧迫しない設計）。
- **ライセンス**: 評価版は無料ダウンロード可。製品版は「AquesTalk ESP32 使用ライセンス」を
  ESP32モジュール1台ごとに購入（MACアドレス紐付け、¥1,980税込/台、AQUESTオンラインストア）。
  製品として販売・頒布する場合は別途頒布ライセンスが必要（今回は個人利用のみなので不要）。

### 所感（実装する場合の主な検討点）
- 価格は¥1,980と個人利用では現実的な範囲。
- 現在のファームは音声出力を無効化している（`M5.Speaker.begin()`コメントアウト、
  `USE_VOICE_TEXT`未定義）。音声合成を有効化するには、スピーカー出力パイプラインの
  復活＋AquesTalkライブラリ組み込み＋既存のJSON `speech`コマンドから音声合成呼び出しへの
  配線、という追加実装が必要（今回のBalloon.h修正よりまとまった作業量になる見込み）。
- 既存のWiFi/サーボ機能との共存（メモリ・処理負荷）は未検証。
- 実装する場合は、上記の「価格・ライセンス購入（MACアドレス紐付け）」を先に確定させる
  必要がある。

Sources:
- [AquesTalk ESP32 - ESP32モジュール用 音声合成ライブラリ](https://www.a-quest.com/products/aquestalk_esp32.html)
- [AquesTalk ESP32 使用ライセンス | AQUEST Online Store](https://store.a-quest.com/items/10524168)
...
## 音声合成(VOICEVOX)：Claude Code通知の実装完了（2026-07-29）

「事前生成した固定フレーズをCore2内蔵フラッシュに埋め込んで再生する」方式で、
Claude Code通知（done/ask/error）の音声対応が完成した。将来Raspberry Piに
常時稼働タスクを移す計画があるため、**実行時にリアルタイム合成が不要な設計**を採用。

### アーキテクチャ
- **事前準備（PC上で1回だけ）**: VOICEVOXで各フレーズを合成 → WAV → Cヘッダに変換 → Core2に埋め込み
- **実行時**: Pi/PCは`{"sound":"done"}`のような小さなJSONコマンドを送るだけ。
  Core2は該当ファイルを**自分のフラッシュから`M5.Speaker.playWav()`で再生**する。
  WiFi経由の音声データ転送もPi側のリアルタイム合成も不要（Piの負荷はほぼゼロ）。
- 数値を含むフレーズ（気温・株価等）も「取りうる値の全パターンを事前生成」すれば
  同じ仕組みで扱える。ファーム変更は不要でファイルを足すだけ。

### 実装内容
- **`tools/generate_voices.py`**: VOICEVOXのローカルAPI(`/audio_query`→`/synthesis`)を
  呼び、`assets/voices/*.wav`を生成。Core2のRAM節約のため16kHzモノラルで出力。
  話者はずんだもん(ノーマル, speaker id=3)。
- **`tools/wav_to_header.py`**: WAVをC配列に変換し`voices.h`を自動生成
  （`base_firmware/`と`firmware/`の両方に出力）。名前→データのテーブル`kVoiceTable`を持つ。
- **`.ino`**: JSONに`sound`キーを追加。`playVoice(name)`で`kVoiceTable`から検索して再生。
  **`audioEnabled`（BtnAでトグル、既定OFF）でゲート**しており、OFF中は音が鳴らない。
- **`stackchan_notify.py`**: `--event done/ask/error`は対応する`sound`も自動的に送る
  （音声名はイベント名と同じにしてある）。`speak()`にも`sound=`引数を追加。
- **音量調整**: BtnB=ダウン, BtnC=アップ（10%刻み, `M5.Speaker.setVolume()`）。
  変更後は吹き出しで音量%を表示し、確認用ビープ音をその音量で鳴らす。

### VOICEVOXエンジンの起動（headless、PC上）
GUIなしでAPIサーバーだけ起動する場合の注意点：
- 実行ファイル: `C:\Program Files\VOICEVOX\vv-engine\run.exe`
  （インストール先はGUIインストーラーでも環境により`Program Files`直下になることがある。
  `%LOCALAPPDATA%\Programs\VOICEVOX`ではない場合がある）
- **既定ポート50021は動的ポート範囲(49152〜)外だが、他プロセスに使われていることがある**。
  さらに49152以上の値を指定すると、Windowsの動的ポート範囲と重なり他アプリに
  奪われる可能性がある。`--port`で49152未満の値（例: 9021）を明示指定するのが安全。
  起動コマンド例: `run.exe --port 9021 --host 127.0.0.1`
- 起動時に辞書の読み込みで数秒〜十数秒かかる。`/version`エンドポイントが応答するまで
  ポーリングして待つとよい。

### 実機確認結果
done/ask/errorの3イベントとも、表情+セリフ+モーション+音声が同時に正しく発火することを
確認。BtnAでのON/OFFトグル、BtnB/BtnCでの音量調整もすべて実機確認済み。

### 教訓
- ハードウェアのテスト結果は、**曖昧・不自然に見える入力をそのまま「確認済み」として
  扱わない**こと。今回、文字化けらしきノイズを含むメッセージを誤って実際のテスト結果
  として扱い、未検証のまま「完了」と報告してしまう場面があった。特に安全確認を伴う
  実機テストでは、疑わしい場合は必ず再度明確な確認を取ること。

### 今後の発展候補
- 株取引・多肉植物モニターへの拡張（数値パターンの事前生成を含む）
- 将来的にフレーズを増やす際、容量が大きくなればLittleFS/SDカードへの移行と、
  WiFi経由でのフレーズ追加の仕組み（USB差し替え不要にする）を検討

---

## 他プロジェクトからのフレーズ/アクション依頼の仕組み化（2026-07-30）

株取引・多肉植物モニター側で「新しいフレーズ/アクションが欲しい」となった際、ユーザーが
手動で伝えると抜け漏れが起きうるため、**GitHub Issueをリクエストキューとして使う**運用にした。

### 運用ルール
- ハード（Core2の物理台数・`voices.h`）は1台分しかなく一元管理が必須のため、
  **他プロジェクトはStackChan-kaiのコード/ファームを直接書き換えない**。
- 新しいフレーズ・モーションが必要になったら、他プロジェクト側のClaude Codeがその場で
  `gh issue create --repo kat0216g-bot/Stackchan-Kai --title "..." --body "..."`
  でIssueを立てる（気づいた時点で即Issue化。ユーザーの手動伝達に依存しない）。
- 次にこのプロジェクトで作業する際は `gh issue list --repo kat0216g-bot/Stackchan-Kai`
  で溜まっているリクエストを確認し、対応したらクローズする。

### 展開済みの設定
- `D:\Make\stock_trading_llm\CLAUDE.md`: 既存の「スタックチャン連携」記述に、
  上記Issue化ルールを追記済み。
- `D:\Make\Tanniku_sensor\CLAUDE.md`: 新規作成。同様のルールを記載
  （このプロジェクトはまだスタックチャン連携自体が未実装だが、将来のために先行して記載）。

### 前提インフラの整備
- **GitHub CLI (`gh`) が未インストールだった**ため、`winget install --id GitHub.cli`で導入
  （バージョン2.96.0）。
- `gh auth login`は対話的な操作（デバイスコード認証）が必要なためユーザーに実施してもらった。
  `github.com/login/device` でワンタイムコードを入力する形式。認証後
  `repo`スコープ付きで`kat0216g-bot`としてログイン済みを確認。
- 実際にテストIssueを作成→クローズし、一連の流れが動作することを確認済み。
...
### 追記（同日）: 待機中アクションを音声ON/OFFと連動させる
待機中アクションの「軽い動き（Tilt）」が、音声ON/OFFに関わらず常に動いていた点を修正。
`maybeIdleChatter()`内の`playVoice()`/`playMotion()`呼び出しを`if (audioEnabled)`で
まとめてゲートし、**音声OFF中は表示（表情・つぶやき文字）のみ**、**音声ONで表示＋動き＋音声**
という仕様にした。通知（done/ask/error/stock_signal等）側のモーションはこれとは独立で、
audioEnabledに関わらず常に動く（待機中アクションだけを対象とした変更）。
実機で音声OFF/ON両パターンとも意図通りの挙動になることを確認済み。

---

## 新モーション Greet（挨拶）・Laugh（笑う）を追加（2026-08-XX）

「表情・モーションのパターンを増やしたい」との要望を受け、公式`stack-chan-arduino`
（`Stackchan_servo.cpp`）が持つ定型モーション一覧（`nomove`/`greet`/`laugh`/`nod`/`refuse`）を
調査。表情は`m5stack-avatar`の`Expression.h`で`Happy/Angry/Sad/Doubt/Sleepy/Neutral`の6種が
上限（追加不可、`Angry`のみ未使用）と判明。モーションは`greet`と`laugh`が新規性ありと判断し、
実機のY軸未検証領域を安全確認した上で実装した。

### Y軸下方向（センター75°未満）の安全域を新規探索
これまでY軸は75〜95°（上方向のみ）しか検証していなかった。公式ライブラリの`laugh`が
Y=60を使うため、1〜5°刻みで73→70→65→60→55→50と段階的に検証し、**50°まで異音なく
安全に動作すること**を確認した（`SERVO_Y_MIN`を75→50に更新）。

### 実装内容
- `Motion`enumに`Greet`/`Laugh`を追加。`parseMotion()`で`"greet"`/`"laugh"`を解釈。
- **Greet（挨拶）**: Y軸 75→90→75 を**1回だけゆっくり**（うなずきとの違いは回数と間の`delay(300)`）。
  既存の`NOD_Y_DOWN`定数をそのまま流用。
- **Laugh（笑う）**: Y軸 80⇔60 を**5回素早く往復**。公式ライブラリと**全く同じ角度**を使用
  （このユニットのY中心が偶然にも公式の想定と同じ75°だったため、数値をそのまま流用できた）。
- `stackchan_notify.py`の`VALID_MOTIONS`に`"greet"`/`"laugh"`を追加。
- 実機でGreet・Laugh両方とも異音なく正常動作することを確認済み。

### 使い道は未定
今回は「パターンを増やす」こと自体が目的だったため、Greet/Laughをどのイベントに
割り当てるかは未着手。既存の3イベント（done/ask/error）は現状のモーションのままとし、
将来別の通知（例: 挨拶イベント、褒め言葉的な通知等）が生まれた際に活用する想定。
